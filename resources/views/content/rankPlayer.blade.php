    ...
    <script>
        $(document).ready( function () {
            ...
            $(document).on('change', '#date_picker', function (e) {
                ...
                table.ajax.url('{{ route('api.worldPlayer', ['de',170]) }}/' + $(this).val()).load();
            });
            ...
            $('#table_id_wrapper').prepend('' +
                ...
                '<input id="date_picker" class="form-control form-control-sm col-8" type="date" value="{{ \Illuminate\Support\Carbon::now()->subDay()->toDateString() }}" max="{{ \Illuminate\Support\Carbon::now()->subDay()->toDateString() }}" min="{{ \Illuminate\Support\Carbon::now()->subDays(30)->toDateString() }}">' +
                '</div>' +
                '<div class="col-4">' +
                '</div>' +
                '</div>');
