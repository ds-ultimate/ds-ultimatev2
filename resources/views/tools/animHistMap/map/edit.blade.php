...
<script>
    ...
    function titleSave() {
        ...
        axios.post('{{ route('tools.animHistMap.modePost', [$wantedMap->id, 'title', $wantedMap->edit_key]) }}', {
            'title': t
        })
            .then((response) => {
                input.hide();
    ...
    function addCustomLibs(context) {
        ...
        $('.select2-player', context).select2({
            ajax: {
                url: '{{ route("api.select2PlayerTop", [$worldData->server->code, $worldData->name]) }}',
        ...
        $('.select2-ally', context).select2({
            ajax: {
                url: '{{ route("api.select2AllyTop", [$worldData->server->code, $worldData->name]) }}',
                data: function (params) {
                    var query = {
                        search: params.term,
                        page: params.page || 1
